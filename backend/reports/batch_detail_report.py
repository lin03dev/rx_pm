...
class BatchDetailReport(BaseReportV2):
    ...
    10. Daily Updates - Session-wise updates
    11. Timetable/Sessions - Schedule
    12. Trainers & Mentors - Assigned staff
    13. Batch Comparisons - Compare with other batches
    """
    ...
    def _get_daily_updates(self) -> pd.DataFrame:
        """Session details from registered LMS tables (sessions/module)."""
        query = f"""
        SELECT 
            ...
            m.module as "Module"
        FROM sessions s
        LEFT JOIN module m ON s.batchmodule = m.id
        ...
        return self.schema.query_optional(
            query,
            ["sessions", "module"],
            message="Daily session updates unavailable",
        )
    
    def _get_timetable(self) -> pd.DataFrame:
        """Get timetable/sessions for this batch using registered LMS tables."""
        query = f"""
        SELECT 
            TO_CHAR(s.date, 'YYYY-MM-DD') as "Date",
            ...
            m.module as "Module"
        FROM sessions s
        LEFT JOIN batchmodule bm ON s.batchmodule = bm.id
        LEFT JOIN module m ON bm.moduleid = m.id
        WHERE s.batch = {self.batch_id}
        ORDER BY s.date
        """
    ...
    def _get_trainers_mentors(self) -> pd.DataFrame:
        ...
        JOIN person p ON e.student = p.id
        WHERE e.batch = {self.batch_id}
          AND lower(e.role) IN ('trainer', 'mentor', 'facilitator', 'admin')
        ORDER BY e.role, p.lastname
        """
        return self.schema.query_optional(
            query,
            ["enrollment", "person"],
            message="Trainer and mentor roster unavailable",
        )
...
def generate_batch_report(db_manager, batch_id=None, batch_name=None, output_dir="./output/reports/LMS"):
    ...
    report = BatchDetailReport(
        db_manager,
        batch_id=batch_id,
        batch_name=batch_name,
        report_id="batch-detail",
        db_name=getattr(db_manager, "current_db", None),
    )
    ...
    data = apply_schema_output("batch-detail", data)
    sheet_names = resolve_sheet_names("batch-detail", report.get_sheet_names())
    get_report_excel_writer().save_report(output_path, data, sheet_names, report_id="batch-detail")
