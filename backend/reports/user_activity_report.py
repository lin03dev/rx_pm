...
class UserActivityReport(BaseReport):
    ...
    def generate(self) -> Dict[str, pd.DataFrame]:
        ...
        if not df.empty:
            ...
            # Users with activity in last 60 days
            recently_active_60 = len(df[df['days_inactive'].notna() & (df['days_inactive'] <= 60)])
            summary_data.append({'Metric': 'Active in Last 60 Days', 'Value': recently_active_60})
