...
class LMSReport(BaseReport):
    ...
    def generate(self) -> Dict[str, pd.DataFrame]:
        ...
        try:
            batch_columns = [name.lower() for name in self.schema.table_columns("batch")]
            
            select_fields = ["b.id as batch_id", "b.batch as batch_name"]
            
            if 'location' in batch_columns:
                select_fields.append("b.location")
            if 'startdate' in batch_columns:
                select_fields.append("b.startdate as start_date")
            elif 'start_date' in batch_columns:
                select_fields.append("b.start_date as start_date")
            if 'enddate' in batch_columns:
                select_fields.append("b.enddate as end_date")
            elif 'end_date' in batch_columns:
                select_fields.append("b.end_date as end_date")
            ...
            results['batch_summary'] = self.schema_query(batch_query, ["batch", "enrollment"])
