...
DEFAULT_SYSTEM_CONFIG_FILE = CONFIG_DIR / "system_config.yaml"


class OutputConfig:
    """Configuration for output segregation"""
    
    def __init__(self, config_file: str | Path = DEFAULT_SYSTEM_CONFIG_FILE):
    ...
    def _from_system_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for category, paths in categories_config.items():
            report_categories[category] = {
                "databases": list(paths.get("databases", [])),
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "database_mappings": {
                ...
                "LMS_Dev": {"project_type": "LMS", "output_folder": "LMS", "template_folder": "LMS"},
                "Telios_Dev": {"project_type": "Telios", "output_folder": "Telios", "template_folder": "Telios"}
            },
            "report_categories": {
                "AG": {"databases": ["AG_Dev"]},
                "LMS": {"databases": ["LMS_Dev"]},
                "Telios": {"databases": ["Telios_Dev"]},
                "Language": {"databases": []}
            }
        }
    ...
    def _resolve_storage_path(self, path: str) -> Path:
        """Resolve configured output paths to absolute backend/output locations."""
        if not path:
            return (BACKEND_DIR / "output" / "reports").resolve()
    ...
    def resolve_generated_file(self, path: str) -> Path:
        """Resolve a generated report path for download, regardless of process cwd."""
        ...
        for candidate in candidates:
            ...
            if resolved in seen:
                continue
            seen.add(resolved)
            if output_root not in resolved.parents and resolved.parent != output_root:
                continue
            if resolved.is_file():
                return resolved
        ...
        for candidate in candidates:
            try:
                ...
                continue
            if output_root in resolved.parents or resolved.parent == output_root:
                return resolved

        raise FileNotFoundError(f"Report file not found under {output_root}: {path}")
    ...
    def get_output_path(self, database_name: str) -> str:
        ...
        configured = category_info.get("reports_path") or str(
            Path(self.mappings.get("reports_path", "./output/reports")) / category
        )
    ...
    def get_template_path(self, database_name: str) -> str:
        ...
        category_info = self.get_category_info(category)
        configured = category_info.get("templates_path") or str(
            Path(self.mappings.get("templates_path", "./output/templates")) / category
        )
        return str(self._resolve_storage_path(configured))
