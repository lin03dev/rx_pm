...
class BatchDetailedReport(BaseReportV2):
    ...
    def generate(self) -> Dict[str, pd.DataFrame]:
        ...
        results['survey_responses'] = self._get_survey_responses_detailed()
    ...
    def _get_survey_responses_detailed(self) -> pd.DataFrame:
        try:
            check_query = f"""
            SELECT COUNT(*) as count FROM response 
            WHERE batchid = {self.batch_info['batch_id']}
            """
            check_df = self.execute_query(check_query)
            
            if check_df['count'].iloc[0] == 0:
                return pd.DataFrame({'Message': ['No survey responses for this batch']})
            
            query = f"""
            SELECT 
                s.survey as survey_name,
                COALESCE(p.firstname || ' ' || p.lastname, r.role, 'Unknown') as participant_name,
                r.role as respondent_role,
                COALESCE(q.text, 'Question') as question,
                CASE 
                    WHEN q.questiontype = 1 THEN 'Multiple Choice (Single Answer)'
                    WHEN q.questiontype = 2 THEN 'Multiple Choice (Multiple Answers)'
                    WHEN q.questiontype = 3 THEN 'Free Text / Open Ended'
                    WHEN q.questiontype = 4 THEN 'Rating Scale'
                    WHEN q.questiontype = 5 THEN 'Yes/No'
                    ELSE 'Other'
            ...
            WHERE r.batchid = {self.batch_info['batch_id']}
            ORDER BY s.survey, participant_name, q.surveyorder, q.id
            """
            df = self.execute_query(query)
            
            if df.empty:
                return pd.DataFrame({'Message': ['No detailed survey responses available']})
            
            df = df.fillna('')
            return df
            
        except Exception as e:
            return pd.DataFrame({'Message': [f'Could not retrieve survey data: {str(e)}']})
    ...
    def _get_summary_stats(self, results: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        survey_df = results.get('survey_responses', pd.DataFrame())
        if not survey_df.empty and 'Message' not in survey_df.columns:
            unique_respondents = survey_df['participant_name'].nunique()
            stats.append({'Metric': '', 'Value': ''})
            stats.append({'Metric': '📋 SURVEY OVERVIEW', 'Value': ''})
            stats.append({'Metric': f'  Survey Response Rate', 'Value': f"{round(unique_respondents/total_participants*100, 1)}%"})
            stats.append({'Metric': '  Total Responses', 'Value': len(survey_df)})
    ...
    def get_sheet_names(self) -> Dict[str, str]:
        return {
            'batch_overview': '1. Batch Overview',
            'participants': '2. Participants',
            'module_schedule': '3. Session Schedule',
            'attendance_matrix': '4. Attendance Matrix',
            'attendance_details': '5. Attendance Details',
            'attendance_summary': '6. Attendance Summary',
            'assignment_submissions': '7. Assignment Status',
            'participant_progress': '8. Participant Progress',
            'trainers': '9. Trainers',
            'survey_responses': '10. Survey Responses',
            'summary_stats': '11. Summary Statistics'
        }
